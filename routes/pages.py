...
def history():
    ...
    cur = mysql.connection.cursor()
    cur.execute(
        """SELECT l.*, COALESCE(u.full_name, u.email) AS officer_name, u.role, c.name AS category_name
           FROM audit_logs l
...
@pages_bp.route("/api/notifications")
@login_required
def notifications():
    notifications = get_notifications(mysql, session.get("role", "staff"))
